...
TAV（Think-Act-Verify，思考-执行-验证）是一个面向有范围软件变更的结构化工作流。它将分析、执行、验证三种职责分离，确保每一次非平凡的修改都基于证据、保持最小、并在宣告完成前经过独立检查。

**版本**：3.5.0
**状态**：Stable

权威规范位于 [SKILL.md](SKILL.md)。本 README 是面向人类的概览；schema、命令表、输出契约只在 skill 文件中定义一次，此处仅作引用。
...
3. Actor 只执行计划内的修改。
4. Verifier 审查真实 diff，运行栈适配的检查，排查副作用。
...
| 级别 | 范围 | 工作流 |
|------|------|--------|
| L0 | 微小改动或显而易见的单文件补丁 | 单遍轻量 TAV：引证据、改、跑基线检查。不建状态文件。 |
| L1 | 跨多文件的标准 bug 修复或功能 | 完整的 Thinker -> Actor -> Verifier 工作流 |
| L2 | 架构、迁移、schema、认证改造、分布式流程 | 先 `spec-driven-develop`，再对每个拆分任务应用 TAV |

## 核心特性

- **角色分离**：只读的 Thinker、最小变更的 Actor、从 `git diff` 出发（而非 Actor 汇报）的独立 Verifier；安全敏感或返工两次的变更升级为独立 reviewer agent 验证。
...
- **错误恢复**：计划不匹配返回 Thinker，门禁失败返回 Actor，同一阻塞点失败两次触发 `[PUA-REPORT]` 升级，关键安全问题阻断完成。
- **知识沉淀**：门禁通过后，持久性教训（非显而易见的根因、未记录的命令、依赖坑）沉淀到项目的 `docs/memory/` 目录——每循环最多 1-3 条，由下一次 Thinker 索引召回并再校验。见 [SKILL.md](SKILL.md) Phase 4。
...
- [examples/refactoring.md](examples/refactoring.md) - 行为保持的提取重构，含计划不匹配恢复。
...
**TAV Workflow v3.5.0**
*Think-Act-Verify：基于证据的变更、最小化执行、经过验证的完成。*
